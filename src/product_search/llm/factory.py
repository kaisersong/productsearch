"""LLM 工厂，通过配置灵活切换 OpenAI / Anthropic / Ollama。"""
...
def create_llm(config_name: str = "default") -> BaseChatModel:
    ...
    if provider == "openai":
        ...
        raise LLMError(f"不支持的 LLM Provider: {provider}。支持: openai, anthropic, ollama")
...
def _create_openai(settings: LLMSettings, api_key: str, base_url: str | None) -> BaseChatModel:
    ...
    if not api_key:
        raise LLMError("OpenAI API Key 未配置。请设置 OPENAI_API_KEY 环境变量或在 config.toml 中配置。")
